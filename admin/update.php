...
function update_script_selection_form($form, &$form_state) {
  ...
  // Warn the admin if any updates were incompatible.
...
/**
 * Determines if the current admin is allowed to run update.php.
 *
 * @return
 *   TRUE if the current admin should be granted access, or FALSE otherwise.
 */
function update_access_allowed() {
  ...
  // so we fall back on requiring that the admin be logged in as admin #1.
  try {
    require_once DRUPAL_ROOT . '/' . drupal_get_path('module', 'user') . '/contact.module';
...
// Determine if the current admin has access to run update.php.
...
// Only proceed with updates if the admin is allowed to run them.
if (update_access_allowed()) {
  ...
  // no errors, skip reporting them if the admin has provided a URL parameter
