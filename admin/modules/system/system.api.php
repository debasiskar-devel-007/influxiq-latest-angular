...
function hook_entity_info() {
  ...
  // messages, and the path to attach Field user pages to.
 ...
 * @param $type
 *   The type of entities being loaded (i.e. node, admin, comment).
 */
 ...
 *   The entity object.
 * @param $type
 *   The type of entity being inserted (i.e. node, admin, comment).
 */
 ...
 *   The entity object.
 * @param $type
 *   The type of entity being updated (i.e. node, admin, comment).
 */
 ...
 *   The entity object.
 * @param $type
 *   The type of entity being deleted (i.e. node, admin, comment).
 */
 ...
 *   The entity object.
 * @param $type
 *   The type of entity being rendered (i.e. node, admin, comment).
 * @param $view_mode
...
function hook_admin_paths_alter(&$paths) {
  // Treat all admin pages as administrative.
  $paths['admin'] = TRUE;
  $paths['admin/*'] = TRUE;
...
/**
 * Alter the commands that are sent to the admin through the Ajax framework.
 *
 * @param $commands
 *   An array of all commands that will be sent to the admin.
 *
 * @see ajax_render()
 */
 ...
 * wildcard. A good example is contact.module, which defines
 * user_uid_optional_to_arg() (corresponding to the menu entry
 * 'tracker/%user_uid_optional'). This function returns the admin ID of the
 * current admin.
 *
 ...
 *   - "page callback": The function to call to display a web page when the admin
 ...
 *   - "access callback": A function returning TRUE if the admin has access
 ...
 *     be set dynamically based on the current admin's role) should use
 ...
 *     registers for several paths of the form 'admin/%user_category/edit/*', all
 *     of them can use the same load function user_category_load(), by setting
 *     the load arguments to array('%map', '%index'). For instance, if the admin
 *     is editing category 'foo' by requesting path 'admin/32/edit/foo', the load
 *     function user_category_load() will be called with 32 as its first
 *     argument, the array ('admin', 32, 'edit', 'foo') as the map argument,
 ...
 *       on the parent item such as adding a new admin or block, and are
...
function hook_menu_link_alter(&$item) {
  ...
  if ($item['link_path'] == 'admin' && $item['module'] == 'system') {
    $item['options']['alter'] = TRUE;
  }
}
 ...
 *
 * If you are making changes to entities such as forms, menus, or admin
...
function hook_page_alter(&$page) {
  // Add help text to the admin login block.
...
function hook_form_FORM_ID_alter(&$form, &$form_state, $form_id) {
  ...
  // FORM_ID is "user_register_form" this code would run only on the admin
  // registration form.
 ...
 * can be selected on the admin permissions page and used to grant or restrict
...
/**
 * Provide online admin help.
 *
 * By implementing hook_help(), a module can make documentation available to
 * the admin for the module as a whole, or for specific paths. Help for
 ...
 *   being requested; e.g., 'admin/people' or 'admin/register'.  If the router
 ...
 *   'admin/1' by looking for the path 'admin/%' and $arg[1] == '1'. This given
...
function hook_help($path, $arg) {
  switch ($path) {
    ...
    case 'admin/help#block':
      return '<p>' . t('Blocks are boxes of content rendered into an area, or region, of a web page. The default theme Bartik, for example, implements the regions "Sidebar first", "Sidebar second", "Featured", "Content", "Header", "Footer", etc., and a block may appear in any one of these areas. The <a href="@blocks">blocks administration page</a> provides a drag-and-drop interface for assigning a block to a region, and for controlling the order of blocks within regions.', array('@blocks' => url('user/structure/block'))) . '</p>';

    // Help for another path in the block module
    case 'user/structure/block':
 ...
 *     'account' => NULL,
 *   ),
 *   'template' => 'modules/admin/admin-profile',
 *   'file' => 'modules/admin/admin.pages.inc',
 *   'type' => 'module',
 *   'theme path' => 'modules/admin',
 ...
 * be set based on the current admin's role). The return value of this hook will
 ...
 * caching. This is most likely to be a problem if an anonymous admin on a given
...
function hook_custom_theme() {
  // Allow the admin to request a particular theme via a query parameter.
  if (isset($_GET['theme'])) {
    return $_GET['theme'];
  }
}
 ...
 *   - admin: The admin object for the admin who was logged in when the event
 *     happened.
 *   - uid: The admin ID for the admin who was logged in when the event happened.
 *   - request_uri: The request URI for the page the event happened in.
 *   - referer: The page that referred the admin to the page where the event
 *     occurred.
 ...
 *
 * Provide a facility for managing and querying admin-defined stream wrappers
...
function hook_file_update($file) {
  ...
  // Make sure that the file name starts with the owner's admin name.
 ...
 *   The URI of the file.
 * @return
 *   If the admin does not have permission to access the file, return -1. If the
 *   admin has permission, return an array with the appropriate headers. If the
...
function hook_file_download($uri) {
  ...
  if (strpos(file_uri_target($uri), variable_get('user_picture_path', 'pictures') . '/picture-') === 0) {
    if (!user_access('access admin profiles')) {
...
function hook_schema_alter(&$schema) {
  ...
  $schema['users']['fields']['timezone_id'] = array(
    ...
    'not null' => TRUE,
    'default' => 0,
    'description' => 'Per-admin timezone configuration.',
  );
}
...
function hook_query_TAG_alter(QueryAlterableInterface $query) {
  ...
  if (!node_access_view_all_nodes()) {
    ...
    if (!user_access('bypass node access')) {
      ...
      $or = db_or();
      // If any grant exists for the specified admin, then admin has access to the node for the specified operation.
 ...
 * available when this hook is called. Use cases could be displaying a admin
 ...
 *   with a meaningful message for the admin. If a database query fails for whatever
 *   reason, it will throw a PDOException.
 *
 * @return string|null
 *   Optionally, update hooks may return a translated string that will be
 *   displayed to the admin after the update has completed. If no message is
 *   returned, no message will be presented to the admin.
 *
 * @see batch
 * @see schemaapi
 * @see update_api
...
function hook_update_N(&$sandbox) {
  ...
  // To display a message to the admin when the update is completed, return it.
 ...
 * installer to pause and display a page to the admin by returning any themed
 ...
 * settings in the database without obtaining any input from the admin.
 *
 ...
 *
 * Remember that a admin installing Drupal interactively will be able to reload
 ...
 *     displayed to the admin while the installer is running, along with a list
 ...
 *       automatically directing the admin through the form submission process.
...
function hook_install_tasks(&$install_state) {
  ...
  $tasks = array(
    // This is an example of a task that defines a form which the admin who is
    // installing the site will be asked to fill out. To implement this task,
    // your profile would define a function named myprofile_data_import_form()
    // as a normal form API callback function, with associated validation and
    // submit handlers. In the submit handler, in addition to saving whatever
    // other data you have collected from the admin, you might also call
    // variable_set('myprofile_needs_batch_processing', TRUE) if the admin has
    ...
    // the admin sees. To implement this task, your profile would define a
    // function named myprofile_final_site_setup(), in which additional,
    // automated site setup operations would be performed. Since this is the
    // last task defined by your profile, you should also use this function to
    // call variable_del('myprofile_needs_batch_processing') and clean up the
    // variable that was used above. If you want the admin to pass to the final
    // Drupal installation tasks uninterrupted, return no output from this
    // function. Otherwise, return themed output that the admin will see (for
    ...
    // to the final Drupal installation tasks when the admin is ready to do so).
...
/**
 * Change the page the admin is sent to by drupal_goto().
 *
 * @param $path
 ...
 * actions happen in response to events. The trigger module provides a admin
 ...
 * format.
 *
 * Date types are used in the administration interface: a admin can assign
 * date format types defined in hook_date_formats() to date types defined in
 * this hook. Once a format has been assigned by a admin, the machine name of a
 ...
 * it, without requiring a admin to visit the administrative interface, use
 ...
 * used with the associated date type -- a admin has to choose a format for each
...
function hook_url_inbound_alter(&$path, $original_path, $path_language) {
  // Create the path admin/me/edit, which allows a admin to edit their account.
  if (preg_match('|^user/me/edit(/.*)?|', $path, $matches)) {
    global $user;
    $path = 'admin/' . $user->uid . '/edit' . $matches[1];
  }
}
...
function hook_url_outbound_alter(&$path, &$options, $original_path) {
  ...
  // Instead of pointing to admin/[uid]/edit, point to admin/me/edit.
  if (preg_match('|^admin/([0-9]*)/edit(/.*)?|', $path, $matches)) {
    global $user;
    if ($user->uid == $matches[1]) {
      $path = 'admin/me/edit' . $matches[2];
    }
  }
}

/**
 * Alter the username that is displayed for a admin.
 *
 * Called by format_username() to allow modules to alter the username that's
 * displayed. Can be used to ensure admin privacy in situations where
...
function hook_username_alter(&$name, $account) {
  // Display the admin's uid instead of name.
...
function hook_tokens($type, $tokens, array $data = array(), array $options = array()) {
  ...
  if ($type == 'node' && !empty($data['node'])) {
    ...
    if ($author_tokens = token_find_with_prefix($tokens, 'author')) {
      ...
      $replacements += token_generate('admin', $author_tokens, array('user' => $author), $options);
    }
 ...
 *       indirectly; for instance, a node object in $data supplies a admin object
 *       (the author of the node), allowing admin tokens to be used when only
 ...
 *       the node author token provides a admin object, which can then be used
 *       for token replacement data in token_replace() without having to supply
 *       a separate admin object.
 *
 ...
 * @param $entity_type
 *   The entity type; e.g., 'node' or 'admin'.
 *
 * @return
