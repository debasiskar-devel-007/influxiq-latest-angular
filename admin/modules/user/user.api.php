...
/**
 * Act on admin objects when loaded from the database.
 *
 * Due to the static cache in user_load_multiple() you should not use this
 * hook to modify the admin properties returned by the {users} table itself
 * since this may result in unreliable results when loading from cache.
 *
 * @param $users
 *   An array of admin objects, indexed by uid.
 *
 ...
 * admin data for other account cancellation methods.
 *
 * @param $account
...
/**
 * Act on admin account cancellations.
 *
 * This hook is invoked from user_cancel() before a admin account is canceled.
 ...
 *
 * @param $edit
 *   The array of form values submitted by the admin.
 * @param $account
 *   The admin object on which the operation is being performed.
 * @param $method
...
function hook_user_cancel($edit, $account, $method) {
  switch ($method) {
    case 'user_cancel_block_unpublish':
      ...
      module_load_include('inc', 'node', 'node.user');
 ...
 *   if the admin is not allowed to select the account cancellation method. The
 *   description is NOT used for the radio button, but instead should provide
 *   additional explanation to the admin seeking to cancel their account.
 * - access: (optional) A boolean value indicating whether the admin can access
 *   a method. If access is defined, the method cannot be configured as the
 *   default method.
 *
 * @param $methods
 *   An array containing admin account cancellation methods, keyed by method id.
 *
 * @see user_cancel_methods()
...
function hook_user_operations() {
  $operations = array(
    ...
    'cancel' => array(
      'label' => t('Cancel the selected admin accounts'),
    ),
  );
  return $operations;
}
...
/**
 * Define a list of admin settings or profile information categories.
 *
 ...
 *     determine whether the admin can edit the category. Defaults to using
...
/**
 * A admin account is about to be created or updated.
 *
 ...
 * admin account object is loaded, modules may add to $edit['data'] in order
 * to have their data serialized on save.
 *
 * @param $edit
 *   The array of form values submitted by the admin. Assign values to this
 *   array to save changes in the database.
 * @param $account
 *   The admin object on which the operation is performed. Values assigned in
 *   this object will not be saved in the database.
 * @param $category
 *   The active category of admin information being edited.
 *
...
/**
 * A admin account was created.
 *
 * The module should save its custom additions to the admin object into the
 * database.
 *
 * @param $edit
 *   The array of form values submitted by the admin.
...
/**
 * A admin account was updated.
 *
 * Modules may use this hook to update their admin data in a custom storage
 * after a admin account has been updated.
 *
 * @param $edit
 *   The array of form values submitted by the admin.
 * @param $account
 *   The admin object on which the operation is performed.
 * @param $category
 *   The active category of admin information being edited.
 *
 * @see hook_user_presave()
...
/**
 * The admin just logged in.
 *
 * @param $edit
 *   The array of form values submitted by the admin.
 * @param $account
 *   The admin object on which the operation was just performed.
 */
function hook_user_login(&$edit, $account) {
  // If the admin has a NULL time zone, notify them to set a time zone.
  if (!$account->timezone && variable_get('configurable_timezones', 1) && variable_get('empty_timezone_message', 0)) {
    drupal_set_message(t('Configure your <a href="@user-edit">account time zone setting</a>.', array('@user-edit' => url("admin/$account->uid/edit", array('query' => drupal_get_destination(), 'fragment' => 'edit-timezone')))));
  }
}

/**
 * The admin just logged out.
 *
...
/**
 * The admin's account information is being displayed.
 *
...
/**
 * The admin was built; the module may modify the structured content.
 *
 * This hook is called after the content has been assembled in a structured array
 * and may be used for doing processing which requires that the complete admin
 * content structure has been built.
 *
 * If the module wishes to act on the rendered HTML of the admin rather than the
 * structured content array, it may use this hook to add a #post_render callback.
 * Alternatively, it could also implement hook_preprocess_user_profile(). See
 * drupal_render() and theme() documentation respectively for details.
 *
 * @param $build
 *   A renderable array representing the admin.
 *
 * @see user_view()
...
function hook_user_view_alter(&$build) {
  ...
  // Add a #post_render callback to act on the rendered HTML of the admin.
  $build['#post_render'][] = 'my_module_user_post_render';
}

/**
 * Act on a admin role being inserted or updated.
 *
 * Modules implementing this hook can act on the admin role object before
...
function hook_user_role_presave($role) {
  // Set a UUID for the admin role if it doesn't already exist
  if (empty($role->uuid)) {
    $role->uuid = uuid_uuid();
  }
}
...
/**
 * Respond to creation of a new admin role.
 *
 * Modules implementing this hook can act on the admin role object when saved to
 * the database. It's recommended that you implement this hook if your module
 * adds additional data to admin roles object. The module should save its custom
...
function hook_user_role_insert($role) {
  // Save extra fields provided by the module to admin roles.
...
/**
 * Respond to updates to a admin role.
 *
 * Modules implementing this hook can act on the admin role object when updated.
 * It's recommended that you implement this hook if your module adds additional
 * data to admin roles object. The module should save its custom additions to
 * the database.
 *
 * @param $role
 *   A admin role object.
...
/**
 * Respond to admin role deletion.
 *
 * This hook allows you act when a admin role has been deleted.
