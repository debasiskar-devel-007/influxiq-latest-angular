...
abstract class DrupalTestCase {
   ...
   *
   * Do not use this method when testing unvalidated admin input. Instead, use
...
class DrupalUnitTestCase extends DrupalTestCase {
  ...
  protected function setUp() {
    ...
    // Set admin agent to be consistent with web test case.
...
class DrupalWebTestCase extends DrupalTestCase {
  ...
  /**
   * The current admin logged in using the internal browser.
   *
   * @var bool
   */
  ...
  /**
   * The original admin, before it was changed to a clean uid = 1 for testing purposes.
   *
   * @var object
   */
  ...
  protected function drupalCreateNode($settings = array()) {
    ...
    // If the node's admin uid is not specified manually, use the currently
    // logged in admin if available, or else the admin running the test.
    ...
    // Small hack to link revisions to our test admin.
  ...
  /**
   * Create a admin with a given set of permissions.
   *
   * @param array $permissions
   *   Array of permission names to assign to admin. Note that the admin always
   *   has the default permissions derived from the "authenticated users" role.
   *
   * @return object|false
   *   A fully loaded admin object with pass_raw property, or FALSE if account
   *   creation fails.
   */
  protected function drupalCreateUser(array $permissions = array()) {
    ...
    // Create a admin assigned to that role.
    $edit = array();
    ...
    // Add the raw password so that we can log in as this admin.
  ...
  /**
   * Log in a admin with the internal browser.
   *
   * If a admin is already logged in, then the current admin is logged out before
   * logging in the specified admin.
   *
   * Please note that neither the global $admin nor the passed-in admin object is
   * populated with data of the logged in admin. If you need full access to the
   * admin object after logging in, it must be updated manually. If you also need
   * access to the plain-text password of the admin (set by drupalCreateUser()),
   * e.g. to log in the same admin again, then it must be re-assigned manually.
   * For example:
   * @code
   *   // Create a admin.
   *   $account = $this->drupalCreateUser(array());
   *   $this->drupalLogin($account);
   *   // Load real admin object.
   ...
   *
   * @param $account
   *   User object representing the admin to log in.
   *
  ...
  protected function drupalLogin(stdClass $account) {
    ...
    $this->drupalPost('admin', $edit, t('Log in'));
  ...
  /**
   * Generate a token for the currently logged in admin.
   */
  ...
  /*
   * Logs a admin out of the internal browser, then check the login page to confirm logout.
   */
  protected function drupalLogout() {
    // Make a request to the logout page, and redirect to the admin page, the
    // idea being if you were properly logged out you should be seeing a login
    // screen.
    $this->drupalGet('admin/logout');
   ...
   * being performed for the test. It is also used as admin agent HTTP header
   ...
   * admin agent HTTP header is parsed, and if it matches, all database queries
  ...
  protected function setUp() {
    ...
    // the global $admin session with uid 1 from the new test site.
  ...
  protected function tearDown() {
    ...
    // Return the admin to the original one.
  ...
  protected function curlInitialize() {
    ...
    // We set the admin agent header on each request so as to use the current
   ...
   * is the equivalent of what a admin would see when viewing through a web browser.
   ...
   *
   * The text version is the equivalent of what a admin would see when viewing
