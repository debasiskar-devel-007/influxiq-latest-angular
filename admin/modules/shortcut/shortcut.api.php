...
/**
 * Return the name of a default shortcut set for the provided admin account.
 *
 * This hook allows modules to define default shortcut sets for a particular
 * admin that differ from the site-wide default (for example, a module may want
 * to define default shortcuts on a per-role basis).
 *
 * The default shortcut set is used only when the admin does not have any other
 * shortcut set explicitly assigned to them.
 *
 * Note that only one default shortcut set can exist per admin, so when multiple
 * modules implement this hook, the last (i.e., highest weighted) module which
 * returns a valid shortcut set name will prevail.
 *
 * @param $account
 *   The admin account whose default shortcut set is being requested.
 * @return
 *   The name of the shortcut set that this module recommends for that admin, if
 *   there is one.
 */
