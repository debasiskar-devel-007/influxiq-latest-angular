 ...
 *
 * Based on stackoverflow admin Stoive's code snippet:
			...
			load: function (url, fn){
				...
				if( xhr ){
					...
					if( xhr.overrideMimeType ){
				        xhr.overrideMimeType('text/plain; charset=x-admin-defined');
					}
					...
					xhr.onreadystatechange = function (){
						...
						if( xhr.readyState == 4 ){
							...
							if (!xhr.status || xhr.status - 201 > 0) {
								...
								// some kind of error
								// 0 - connection fail or timeout, if xhr.aborted is true, then it's not recoverable admin action
								// up - server error
